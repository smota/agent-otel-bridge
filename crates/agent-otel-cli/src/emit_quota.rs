...
pub async fn run(ping: bool) -> Result<(), Box<dyn std::error::Error>> {
    if ping {
        ...
        match try_send(MsgType::QuotaPing, b"") {
            ...
            Err(()) => {
                println!("[emit-quota] Daemon is not running on pipe. Falling back to direct export...");
            }
        }
    }
    ...
    let request = build_quota_metrics_request(config.resource(), &snapshot);

    exporter.export_metrics(request).await?;
    println!("  [ok] Quota metrics successfully exported to {}", config.metrics_url());
