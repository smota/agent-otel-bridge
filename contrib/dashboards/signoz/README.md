...
| **AI Tool Archetypes & Capability Waste** | Agent Architects | `v6` | [`tool-archetypes-and-waste.json`](./tool-archetypes-and-waste.json) | [Download JSON](https://raw.githubusercontent.com/smota/agent-otel-bridge/main/contrib/dashboards/signoz/tool-archetypes-and-waste.json) | 10 behavioral archetypes (`filter_compressor`, `state_mutation`, `env_pkg_manager`, etc.), MCP vs Skill breakdown, schema token bloat ranking. |
| **Fleet Operations | FDE** | Forward Deployed Engineers & Fleet Operators | `v6` | [`fleet-operations-fde.json`](./fleet-operations-fde.json) | [Download JSON](https://raw.githubusercontent.com/smota/agent-otel-bridge/main/contrib/dashboards/signoz/fleet-operations-fde.json) | Operational fleet cockpit: behavioral archetypes, active sessions, completed returns, error rates, model tokens, quota resets, and raw event inspector. |
...
Behavioral execution archetypes (`FilterCompressor`, `StructuredParser`, `InspectorDiff`, `SearchRetrieval`, `BuildTestVerify`, `StateMutation`, `EnvPkgManager`, `VcsLifecycle`, `NetworkTransfer`, `GenericExec`) and multi-tier error categories.
