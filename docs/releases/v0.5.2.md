...
Generated hook paths are consistently quoted. Local installation stages both binaries, verifies hashes and preserves rollback state. The development HTTP fixture no longer makes an unnecessary blocking connection while shutting down; regression tests retain the existing cancellation deadline. Parallel fixture workspaces use a unique sequence in addition to timestamps, preventing collisions on hosts with coarser clock resolution.
